...
export interface DailyProgress {
  ...
  speak: number;
  /** cards de vocabulário revisados/aprendidos */
  vocab: number;
}

export const EMPTY_DAILY: DailyProgress = { cards: 0, convos: 0, sentences: 0, speak: 0, vocab: 0 };
...
export function goalForWeek(week: number): DailyProgress {
  return {
    ...
    vocab: vocabForWeek(week).length > 0 ? 5 : 0,
  };
}
...
export function dailyPercent(d: DailyProgress, goal: DailyProgress): number {
  const parts: number[] = [];
  for (const k of ["cards", "convos", "sentences", "speak", "vocab"] as const) {
